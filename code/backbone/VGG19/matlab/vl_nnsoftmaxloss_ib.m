function y = vl_nnsoftmaxloss_ib(x,c,dzdy)
...
% x_sofmax = ex ;
x_sum = sum(ex_var,3);
x_sofmax = ex_var./x_sum;
batch_size = size(x,4);
num_class = size(x,3);
a = gpuArray(zeros(num_class, batch_size+1));

for k = 1:batch_size
    [max_value,b] = max(x_sofmax(:,:,:,k));
    
    a(b,1) = a(b,1)+1;
    index_new = a(b,1)+1;
    a(b,index_new) = max_value;
end
...
for j = 1:num_class
   if a(j,1)>1
       class_score = a(j,2:1+a(j,1));
       var_j = var(class_score,1);

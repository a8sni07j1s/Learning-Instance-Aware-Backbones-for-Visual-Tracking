...
opts.dataDir = fullfile('data','ILSVRC-2012') ;
opts.lite = false ;
...
names_1 = {} ;
% labels_1 = {} ;
for i = dir(fullfile(opts.dataDir, 'images', 'val', 'n*'))'
  [~,lab] = ismember(i.name, cats) ;
  ims = dir(fullfile(opts.dataDir, 'images', 'val', i.name, '*.JPEG')) ;
  names_1{end+1} = strcat([i.name, filesep], {ims.name}) ;
%   labels{end+1} = ones(1, numel(ims)) * lab ;
  fprintf('.') ;
  if mod(numel(names_1), 50) == 0, fprintf('\n') ; end
  %fprintf('found %s with %d images\n', d.name, numel(ims)) ;
end
names_1 = horzcat(names_1{:}) ;
% labels_1 = horzcat(labels{:}) ;

% ims = dir(fullfile(opts.dataDir, 'images', 'val', '*.JPEG')) ;
names = sort({ims.name}) ;
labels = textread(valLabelsPath, '%d') ;

if numel(names_1) ~= 50e3
...
names = strcat(['val' filesep], names_1) ;

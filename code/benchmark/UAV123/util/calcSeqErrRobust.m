...
if strcmp(results.type,'rect')
    for i = 2:seq_length
        ...
        if (isnan(r) | r(3)<=0 | r(4)<=0)&(~isnan(r_anno))
            results.result(i,:)=results.result(i-1,:);
...
switch results.type
    case 'rect'
        rectMat = results.result;
    case 'ivtAff'
        for i = 1:seq_length
            [rect c] = calcRectCenter(results.tmplsize, results.result(i,:));
            rectMat(i,:) = rect;
            %                     center(i,:) = c;
        end
    case 'L1Aff'
        for i = 1:seq_length
            [rect c] = calcCenter_L1(results.result(i,:), results.tmplsize);
            rectMat(i,:) = rect;
        end
    case 'LK_Aff'
        for i = 1:seq_length
            [corner c] = getLKcorner(results.result(2*i-1:2*i,:), results.tmplsize);
            rectMat(i,:) = corner2rect(corner);
        end
    case '4corner'
        for i = 1:seq_length
            rectMat(i,:) = corner2rect(results.result(2*i-1:2*i,:));
        end
    case 'affine'
        for i = 1:seq_length
            rectMat(i,:) = corner2rect(results.result(2*i-1:2*i,:));
    ...
    case 'SIMILARITY'
        for i = 1:seq_length
            warp_p = parameters_to_projective_matrix(results.type,results.result(i,:));
